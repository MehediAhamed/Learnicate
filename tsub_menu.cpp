...
tsub_menu::tsub_menu(float width, float height) {
...
void tsub_menu::draw(sf::RenderWindow& window) {
...
void tsub_menu::tsub_display(sf::RenderWindow& wind) {
	...
	tsub_menu t_sub(wind.getSize().x, wind.getSize().y);

	TeachSignup t_sign(wind.getSize().x, wind.getSize().y);
	TeachLogin t_login(wind.getSize().x, wind.getSize().y);
		...
		while (wind.pollEvent(tsubevent))
		{
			...
			if (tsubevent.type == sf::Event::KeyReleased) {
				...
				if (tsubevent.key.code == sf::Keyboard::Return) {
					...
					//SignUp:
					if (y == 0) {
						t_sign.SignUpTeacher(wind);

					}
					//Login:
					if (y == 1) {
						t_login.LogInTeacher(wind);
