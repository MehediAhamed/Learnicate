...
sf::Texture ScrollBar::caretUp;
sf::Texture ScrollBar::caretDown;
...
void ScrollBar::setSettings(const sf::Vector2f & position, const sf::Vector2f & size)
{
	...
	sf::Vector2f scrollTotalSize = { scrollArea.getSize().x, size.y - (2 * scrollArea.getSize().x) };
	bar.setSize({ 15,(size.y / targetSize.y) * scrollTotalSize.y });
}
ScrollBar::ScrollBar(const sf::Vector2f & position, const sf::Vector2f & size, sf::View & view, const sf::Vector2f & targetViewSize) : targetView(view)
{
	if (!isTextureLoaded)
	{
		loadTextures();
	...
	scrollArea.setFillColor(sf::Color::Color(230, 230, 230, 255));
	bar.setFillColor(sf::Color::Color(200, 200, 200, 255));
	scrollUpButton.setFillColor(sf::Color::Color(200, 200, 200, 255));
	scrollDownButton.setFillColor(sf::Color::Color(200, 200, 200, 255));
...
void ScrollBar::updateTargetSize(const sf::Vector2f & targetViewSize)
{
	sf::Vector2f scrollTotalSize = { scrollArea.getSize().x, scrollArea.getSize().y - (2 * scrollArea.getSize().x) };
	targetSize = targetViewSize;
...
bool ScrollBar::onScrollUpButton(const sf::Vector2f & point)
{
	return sf::FloatRect(scrollUpButton.getPosition(), scrollUpButton.getSize()).contains(point);
}
bool ScrollBar::onScrollDownButton(const sf::Vector2f & point)
{
	return sf::FloatRect(scrollDownButton.getPosition(), scrollDownButton.getSize()).contains(point);
}
bool ScrollBar::onBar(const sf::Vector2f & point)
{
	return sf::FloatRect(bar.getPosition(), bar.getSize()).contains(point);
}
bool ScrollBar::contains(const sf::Vector2f & point)
{
	return sf::FloatRect(scrollArea.getPosition(), scrollArea.getSize()).contains(point);
}