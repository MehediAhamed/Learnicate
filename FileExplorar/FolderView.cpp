...
FolderView::FolderView(sf::Vector2f size)
{
...
std::string FolderView::getCurrentPath() const
{
    
	return currentPath;
}

std::string FolderView::getSelectedFilePath() const
{
	if (selectedFile != -1)
	{
		std::cout << files[selectedFile].getPath() << std::endl;
		return files[selectedFile].getPath();
	}
	return "";
}

std::string FolderView::getSelectedFileName() const
{
	if (selectedFile != -1)
		
	{
		std::cout << files[selectedFile].getName();
		return files[selectedFile].getName();
	}
	return "";
}

void FolderView::loadDirectory(const std::string path, bool effectRibbon)
{
	selectedDrive = -1;
	selectedFile = -1;
	if (path == "")
...
sf::FloatRect FolderView::getGlobalBounds() const
{
	return globalBounds;
}

Control * FolderView::handleEvent(sf::Event evnt, sf::Vector2f mousePos)
{
	switch (evnt.type)
	{
...
void FolderView::draw(sf::RenderTarget & target, sf::RenderStates states) const
{
...
void FolderView::onClickLeft(const sf::Event::MouseButtonEvent & , sf::Vector2f mousePos)
{
	...
	if (currentPath == "")
	{
		...
		for (int i = 0; i < derivesCount; i++)
		{
			if (drives[i].contains(mousePos))
			{
				...
				{
					if (selectedDrive != -1)
						drives[selectedDrive].setBackgroundColor(sf::Color::Transparent);
					selectedDrive = i;
					drives[i].setBackgroundColor(sf::Color(51, 198, 84, 200));
					lastClickTime.restart();
				}
	...
	else
	{
		...
		for (int i = 0; i < filesCount; i++)
		{
			if (files[i].contains(mousePos))
			{
				...
				{
					if (selectedFile != -1)
						files[selectedFile].setBackgroundColor(sf::Color::Transparent);
					selectedFile = i;
					files[selectedFile].setBackgroundColor(sf::Color(51, 198, 84, 200));
					lastClickTime.restart();
				}
				...
				{
					chatbox r;
					r.file_network_send(files[i].getPath(), files[i].getName(),files[i].getExtention(),r.material_file_port_text);
					/// <summary>
					/// //////
					/// </summary>
					/// <param name=""></param>
...
{
	std::string newPath = currentPath.substr(0, currentPath.find_last_of('\\', currentPath.length() - 2) + 1);
	loadDirectory(newPath);
}

void FolderView::onMouseMove(const sf::Event::MouseMoveEvent& mouse, sf::Vector2f mousePos)
{
	if (currentPath == "")
	{
		int len = drives.size();
		for (int i = 0; i < len; i++)
		{
			...
			{
				drives[i].setBackgroundColor(sf::Color(51,190,198,150));
			}
			else
			{
				drives[i].setBackgroundColor(sf::Color::Transparent);
			}
		}
	}
	else
	{
		...
		for (int i = 0; i < len; i++)
		{
			if (selectedFile == i)
			{
				files[i].setBackgroundColor(sf::Color(51, 198, 84, 200));
			}
			else if (files[i].contains(mousePos))
			{
				files[i].setBackgroundColor(sf::Color(51, 190, 198, 150));
			}
			else
			{
				files[i].setBackgroundColor(sf::Color::Transparent);
			}
		}
	}
}
