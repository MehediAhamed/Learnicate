...
sf::Texture File::fileIcons[2];
sf::Font File::fileFont[2];
bool File::isResourcesLoaded = false;

File::File(const std::string path, const std::string FileName, const Type t, unsigned long long int size) :
...
void File::draw(sf::RenderTarget & target, sf::RenderStates states) const
{
	...
	if (type != File::Folder)
	{
		sf::Text ext(extension, fileFont[1], characterSize-10);
		ext.setPosition(thisPos.x+5,thisPos.y+25);
		ext.setFillColor(sf::Color::Magenta);
		target.draw(ext);
	...
	sf::Text nameTxt(nameDisplay, fileFont[0], characterSize);
	...
	sf::Text sizeTxt(sizeString, fileFont[0], characterSize-5);
...
Control * File::handleEvent(sf::Event event, sf::Vector2f mousePos)
{
	return this;
}
...
void File::setSizeString()
{
	...
	if (tempSize < 1024)
	{
		sizeString = std::to_string(tempSize) + " Bytes";
	}
	else if (tempSize < 1048576)
	{
		sizeString = std::to_string(tempSize / 1024) + " KB";
	}
	else if (tempSize < 1073741824)
	{
		sizeString = std::to_string(tempSize / 1048576) + " MB";
	}
	else
	{
		std::stringstream ss;
		ss << std::setprecision(1) << std::fixed << tempSize / 1073741824.0 << " GB";
		sizeString = ss.str();
	}
}
...
void File::toUpper(std::string & s)
{
	int len = s.length();
	for (int i = 0; i < len; i++)
	{
