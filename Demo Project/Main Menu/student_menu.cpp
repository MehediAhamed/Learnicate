...
void student_menu::stu_chat()
{
	chatbox r;
	std::vector<std::thread> thr;

	std::thread t1(std::thread([&] { r.s_receiveClient(); })), t2(std::thread([&] { r.s_sendClient(); }));
	t1.join();
	t2.join();
...
void student_menu::stu_video_chat()
{
	...
	std::thread t1(std::thread([&] { r.teacher_camera_video(); })), t2(std::thread([&] { r.teacher_show_video(); })),
		t3(std::thread([&] { r.t_a_receiveClient(); })), t4(std::thread([&] { r.t_a_sendClient(); }));
	t1.join();
	t2.join();
	t3.join();
	t4.join();
